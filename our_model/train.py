...
from models.ourmodel2 import OurModelBertClassifier, Myloss, Myloss2, init_weights
...
class Instructor:
    ...
    @profile(precision=4, stream=open('test.log', 'w+'))
        def _train(self, criterion, my_criterion, weight_criterion, optimizer, max_test_acc_overall=0):
        max_test_acc = 0  
        max_f1 = 0  
        global_step = 0  
        model_path = ''
        train_loss_list, train_acc_list, test_acc_list, test_f1_list, weight_loss_list = [], [], [], [], []
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                my_loss_sta_ave_tensor = (my_loss_sta_ave_tensor + outputs[-2].detach().cpu().numpy())/2 
                ...
                if self.opt.losstype is not None:
                    ...
                    loss4 = weight_criterion(sum(outputs[-1])/2)
                ...
                weight_loss_list.append(loss4.item())

                '''lambada*bgcn+attention'''
                # loss = self.opt.lambda_1*loss1 + loss2 + self.opt.lambda_2*loss3
                loss = loss2 + self.opt.lambda_2 * loss3 + self.opt.lambda_3 * loss4
                # loss = self.opt.lambda_1*loss1+loss2
                loss.requires_grad_()

                # print(log.blue(f'epoch{epoch+1}'),': th-',log.green(f'{i_batch+1}'),'main_loss为',\
                #       log.red(f'{loss2}'),'kl_c_loss(*lambda_2)为:',log.red(f'{loss3*self.opt.lambda_2}'),\
                #       'weight_loss(*lambda_3)为:',log.red(f'{loss4*self.opt.lambda_3}'),'all_Loss为:',log.magenta(f'{loss}'))
                # print(inputs[0].size())
                ...
                if global_step % self.opt.log_step == 0: 
                        ...
                        if test_acc > max_test_acc_overall:  
                            ...
                            if self.opt.dataset == 'restaurant':
                                if test_acc > self.opt.restaurant_acc:
                                    engine.say(f"max_acu:  {round(100*test_acc,3)}")
                                    engine.runAndWait()
                    ...
                    logger.info(log.yellow(
                        '  {}   loss: {:.4f}, loss{:.8f},loss{:.8f}, acc: {:.4f}, test_acc: {:.4f}, f1: {:.4f}'.format(global_step / self.opt.log_step,loss.item(),loss3.item(),loss4.item(), train_acc,
                                                                                         test_acc, f1)))

                    train_loss_list.append(loss3.item())
        ...
        result_dict = {'train_loss_list': train_loss_list,
                       ...
                       'weight_loss_list':weight_loss_list}
    ...
    def _evaluate(self, show_results=False):
        ...
        if show_results:  
            report = metrics.classification_report(labels, predic,
                                                   digits=4)  
    ...
    def run(self):
        ...
        weight_criterion = Myloss2(self.opt).to(self.opt.device) 
        ...
        max_test_acc, max_f1, model_path, result_dict = self._train(criterion, my_criterion, weight_criterion, optimizer,
...
def _matplotlib(x, *args, **kwargs):
    loss, train_acc, test_acc, f1, weight_loss = x.values()
    plt.figure()
    ...
    plt.figure()
    plt.plot(weight_loss, color='orange', label='weight_loss')
    plt.legend()
    plt.xlabel('each step(5_batch)')
    plt.show()
...
def main():
    ...
    # parser.add_argument('--lambda_1', default=0.31,help='lambda1 )  

    parser.add_argument('--init_weight',default=False, help='init_wei-；ght')

    #model2_new_parameter
    parser.add_argument('--trac_att_k', default=20, help='tracking_att_num')  
    parser.add_argument('--lambda_2', default=0.32, help='lambda2 → my_loss loss') 
    parser.add_argument('--lambda_3', default=0.01, help='noise_weight')  



    parser.add_argument('--save_laptop_acc',default=0.82, help='0laptop_base_acc')
    parser.add_argument('--save_restaurant_acc',default=0.874, help='restaurant_base_acc')
    parser.add_argument('--save_twitter_acc',default=0.779, help='twitter_base_acc')
    ...
    parser.add_argument('--twitter_acc',default=0.779, help='say_twitter_acc')
    ...
    if not os.path.exists('log(model2）'):
        os.makedirs('log(model2）', mode=0o777)  

    log_file = 'track-{}-{}-{}-{}-{}-{}.log(model2）'.format(opt.lambda_2,opt.lambda_3,opt.bgcn_dim,opt.model_name, opt.dataset, strftime("%Y-%m-%d_%H_%M_%S", localtime()))  
    logger.addHandler(logging.FileHandler("%s/%s" % ('./log(model2）', log_file)))  

    ins = Instructor(opt)
    results_dict, acc, f1 = ins.run()

    l_dc_dataframe = pd.DataFrame({'loss':results_dict['train_loss_list']})
    l_tra_dataframe = pd.DataFrame({'loss':results_dict['weight_loss_list']})
